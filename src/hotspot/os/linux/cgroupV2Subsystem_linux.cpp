...
CgroupCpuController* CgroupV2CpuController::adjust_controller(int host_cpus) {
  log_trace(os, container)("Adjusting v2 controller path for cpu: %s", subsystem_path());
  CgroupV2Controller* base_ctrl = static_cast<CgroupV2Controller*>(this);
  ...
  while (cpus == host_cpus && (last_slash = strrchr(cg_path, '/')) != cg_path) {
    *last_slash = '\0'; // strip path
    ...
    path_iterated = true;
    if (cpus != host_cpus) {
      log_trace(os, container)("Adjusted v2 controller path for cpu to: %s", subsystem_path());
      os::free(cg_path);
      os::free(orig);
      return this;
    }
...
CgroupMemoryController* CgroupV2MemoryController::adjust_controller(julong phys_mem) {
  log_trace(os, container)("Adjusting v2 controller path for memory: %s", subsystem_path());
  CgroupV2Controller* base_ctrl = static_cast<CgroupV2Controller*>(this);
  assert(base_ctrl->cgroup_path() != nullptr, "invariant");
  char* orig = os::strdup(base_ctrl->cgroup_path());
  char* cg_path = os::strdup(orig);
  ...
  while (limit < 0 && (last_slash = strrchr(cg_path, '/')) != cg_path) {
    *last_slash = '\0'; // strip path
    ...
    path_iterated = true;
    if (limit > 0) {
      log_trace(os, container)("Adjusted v2 controller path for memory to: %s", subsystem_path());
      os::free(cg_path);
      os::free(orig);
      return this;
    }
